...
def stats_pixelbased(y_true, y_pred):
    ...
    if y_pred.shape != y_true.shape:
        raise ValueError(f"Shape of inputs need to match. Shape of prediction is: {y_pred.shape}.  Shape of y_true is: {y_true.shape}")
...
def ratio(mask1, mask2):
    ...
    return 1.-(num/den) if den != num else 0.
