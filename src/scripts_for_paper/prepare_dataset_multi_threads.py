...
def multi_process_fun(file_list, dirpath, function, num_workers):
    ...
    for worker_num in range(num_workers):
        if worker_num == range(num_workers)[-1]:
            process = multiprocessing.Process(target=function, args=(dirpath, file_list[worker_amount*worker_num:]))   
        else:
...
def process_DA_multi(path_img, path_label, n_workers):  
    ...
    multi_process_fun(list(zip(list_img, list_mask)), (path_img, path_label), wrapped_elastic_transformation, n_workers)
...
def get_args():
    ...
    parser.add_argument(
        "--n_workers", type=int, default=4, help="number of thread used to run the script."
    )
